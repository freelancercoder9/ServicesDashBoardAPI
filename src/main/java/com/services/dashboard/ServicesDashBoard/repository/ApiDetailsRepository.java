...
public interface ApiDetailsRepository extends JpaRepository<ApiDetails, Long> {
    ...
    @Query(value = "select ad.id , ad.api_name ,ad.module_name, ad.app_instance_id , ad.api_request_type ,ad.dev_status_date ,ad.dev_status ,ad.developer_details  ,ad.prod_status_date ,ad.prod_status ,ad.uat_status_date ,ad.uat_status,\n" +
            "coalesce(SUM(ccad.tps_value),0)  as total_tps from api_details ad left join consumer_client_api_details ccad  on ad.id = ccad.api_name_id group by ad.id ", nativeQuery = true)
